...
void GetKejkCredentials(String& login, String& password)
{
	...
	XmlElement* xe = xd.getDocumentElement()->getChildByName(T("kejk"));
...
BEGIN_TEST(Services_KejkService_CanGetCurrent)
{
	KejkService svc;

	String login;
	String password;
	GetKejkCredentials(login, password);
...
BEGIN_TEST(Services_KejkService_CanGetSongUrl)
{
	KejkService svc;
